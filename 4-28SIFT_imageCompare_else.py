...
X_data = []
Y_data = []
files = glob.glob ("/Users/klee22/Downloads/Extract-Frame-from-Videos-main/save/IMG_9290/*.png")
files2 = glob.glob ("/Users/klee22/Downloads/Extract-Frame-from-Videos-main/save/IMG_9291/*.png")

#files = glob.glob ("/Users/klee22/Desktop/save/New_Vid_copy/*.png")
#files2 = glob.glob ("/users/klee22/Desktop/save/New_vid_copy/*.png")
files.sort()
files2.sort()
...
for ref_image in X_data:
  ...
  for new_image in Y_data:
    ...
    bad_points = [m for m, n in matches if m not in good_points]
    #print(bad_points)
    ...
    good_result = cv2.drawMatches(X_data[x], kp_1, Y_data[y], kp_2, good_points, None)
    bad_result = cv2.drawMatches(X_data[x], kp_1, Y_data[y], kp_2, bad_points, None)

    cv2.imshow("good result", cv2.resize(good_result, None, fx=0.4, fy=0.4))
    cv2.imshow("bad result", cv2.resize(bad_result, None, fx=0.4, fy=0.4))

    cv2.imwrite("good_feature_matching.jpg", good_result)
    cv2.imwrite("bad_feature_matching.jpg", bad_result)
